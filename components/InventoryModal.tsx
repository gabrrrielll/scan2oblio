...
interface InventoryModalProps {
  ...
  onClose: () => void;
  onAddToCart: (product: OblioProduct, quantity: number) => void;
}

const InventoryModal: React.FC<InventoryModalProps> = ({ inventory, onClose, onAddToCart }) => {
  ...
  const getQuantity = (code: string) => quantities[code] || 1;

  const handleQuantityChange = (code: string, value: number) => {
    setQuantities(prev => ({ ...prev, [code]: Math.max(1, value) }));
  };
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar bg-slate-900 rounded-b-xl">
          {filtered.length === 0 ? (
            ...
            filtered.map((item, idx) => {
              const itemKey = item.code || item.productCode || `idx_${idx}`;
              return (
                <div key={idx} className="bg-slate-800/40 p-3 rounded-lg border border-slate-800 hover:border-slate-700 transition-colors flex justify-between items-center group">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      {item.productCode && item.productCode.trim() !== '' && (
                        <>
                          ...
                          {item.code && item.code.trim() !== '' && item.code !== item.productCode && <span className="text-xs text-slate-500">|</span>}
                        </>
                      )}
                      {item.code && item.code.trim() !== '' && item.code !== item.productCode && (
                      ...
                      {((item.productCode && item.productCode.trim() !== '') || (item.code && item.code.trim() !== '' && item.code !== item.productCode)) ? (
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex flex-col items-end gap-1 min-w-[80px]">
                      <div className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider border ${item.stock > 0
                    ...
                    <div className="flex items-center gap-2 pl-3 border-l border-slate-700/50">
                      <input
                        type="number"
                        min="1"
                        value={getQuantity(itemKey)}
                        onChange={(e) => handleQuantityChange(itemKey, parseFloat(e.target.value) || 1)}
                        disabled={item.stock <= 0}
                        className={`w-12 bg-slate-950 border border-slate-700 rounded-md py-1 px-1 text-center text-sm text-white focus:ring-1 focus:ring-emerald-500 outline-none ${item.stock <= 0 ? 'opacity-50 cursor-not-allowed' : ''}`}
                      />
                      <button
                        onClick={() => onAddToCart(item, getQuantity(itemKey))}
                        disabled={item.stock <= 0}
                        className={`p-1.5 rounded-lg transition-colors shadow-lg flex items-center justify-center ${item.stock <= 0
                            ? 'bg-slate-700 text-slate-500 cursor-not-allowed'
                            : 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-emerald-500/10'
                          }`}
                        title={item.stock <= 0 ? "Stoc epuizat" : "Adaugă în coș"}
                      >
